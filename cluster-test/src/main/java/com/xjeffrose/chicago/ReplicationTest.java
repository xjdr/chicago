...
public class ReplicationTest {
  ...
  public void testValidResponse(List<String> nodes, String colFam, int key) throws Exception{
    ChicagoClient cc=testChicagoCluster.chicagoClientHashMap.get(nodes.get(0));
    String response1 = new String(cc.read(colFam.getBytes(), Longs.toByteArray(key)).get().get(0));
    cc=testChicagoCluster.chicagoClientHashMap.get(nodes.get(1));
    String response2 = new String(cc.read(colFam.getBytes(), Longs.toByteArray(key)).get().get(0));
    cc=testChicagoCluster.chicagoClientHashMap.get(nodes.get(2));
    String response3 = new String(cc.read(colFam.getBytes(), Longs.toByteArray(key)).get().get(0));
  ...
  public void deleteColFam(String colFam){
    for(ChicagoClient cc : testChicagoCluster.chicagoClientHashMap.values()){
      try {
